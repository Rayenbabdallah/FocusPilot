...
\author{
  \IEEEauthorblockN{FocusPilot Team}
  \IEEEauthorblockA{
    \textit{École Supérieure Privée d'Ingénierie et de Technologie (ESPRIT)}\\
    Tunis, Tunisia\\
...
% ─────────────────────────────────────────────────────────────────────────────
\section{ADHD-Specific Feature Design}
...
\begin{enumerate}
  \item \textbf{Inactivity signal}: No \texttt{mousemove} or \texttt{keydown} events for 90 seconds
  \item \textbf{Visibility signal}: \texttt{document.visibilityState} hidden for 60 cumulative seconds
  \item \textbf{Scroll thrash signal}: More than 8 \texttt{scroll} events within any 4-second window
\end{enumerate}

On signal detection, the hook calls \texttt{POST /sessions/\{id\}/drift}, which triggers Bedrock to generate a recall question from the current chunk. The question is displayed as a re-anchoring overlay requiring active engagement before resumption. Algorithm~\ref{alg:drift} formalizes the detection logic.

\begin{algorithm}
\caption{Behavioral Drift Detection}
\label{alg:drift}
\begin{algorithmic}[1]
\State $t_{\text{inact}} \gets \text{now}()$
\State $t_{\text{hidden}} \gets 0$, $\text{scrollEvents} \gets []$
\Procedure{OnActivity}{}
  \State $t_{\text{inact}} \gets \text{now}()$
  \State $\text{scrollEvents}.\text{push}(\text{now}())$
  \State \textbf{Prune} $\text{scrollEvents}$ where $\text{age} > 4000\text{ms}$
  \If{$|\text{scrollEvents}| > 8$}
    \State \Call{TriggerDrift}{\texttt{scroll\_thrash}}
  \EndIf
\EndProcedure
\Procedure{OnTick}{}
  \If{$\text{now}() - t_{\text{inact}} > 90000$}
    \State \Call{TriggerDrift}{\texttt{inactivity}}
  \EndIf
  \If{$\text{hidden}$ and $t_{\text{hidden}} > 60000$}
    \State \Call{TriggerDrift}{\texttt{tab\_hidden}}
  \EndIf
\EndProcedure
\Procedure{TriggerDrift}{signal}
  \State $q \gets \text{POST } /\text{sessions}/\{id\}/\text{drift}(\text{signal})$
  \State $\text{store.setDrifting}(\text{true})$
  \State $\text{store.setReanchor}(q)$
  \State \textbf{Reset all timers}
\EndProcedure
\end{algorithmic}
\end{algorithm}
